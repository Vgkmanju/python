...
class account(object):
    def acc(self):
        ...
        if yy=="1":
            ...
            pas=str(input())
            con=cx_Oracle.connect("manju1/manju1@127.0.0.1/XE")
            cur=con.cursor()
            sql = "insert into acco VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (self.d1,self.d2,self.d3,self.d4,self.d5,self.d6,val,val1,typ,pas,amo)
            cur.execute(sql)
            con.commit()
            print(" ")
            ...
            print("SET PASSWORD FOR YOUR ACCOUNT!!!!!!!")
            pas=str(input())
            con=cx_Oracle.connect("manju1/manuju1@127.0.0.1/XE")
            cur=con.cursor()
